...
class TestMaksukortti(unittest.TestCase):
    ...
    def test_negatiivisen_arvon_lataaminen_ei_onnistu(self):
        self.kortti.lataa_rahaa(-200)

        self.assertEqual(str(self.kortti), "Kortilla on rahaa 10.00 euroa")

    def test_voi_ostaa_edullisen_lounaan_kun_kortilla_vain_edullisen_lounaan_verran_rahaa(self):
        kortti = Maksukortti(250)
        kortti.syo_edullisesti()

        self.assertEqual(str(kortti), "Kortilla on rahaa 0.00 euroa")

    def test_voi_ostaa_maukkaan_lounaan_kun_kortilla_vain_maukkaan_lounaan_verran_rahaa(self):
